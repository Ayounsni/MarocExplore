...
useEffect(() => {
  axiosClient.get('/api/favoris').then(({data})=> {
    console.log(data);
    setItin(data);
}).catch(() =>{
    ...
    return (
      ...
      <div key={itine.id} className="bg-gray-100 mb-3 px-10 py-4 shadow-md rounded-lg mt-8" >
        ...
        <h1 className="text-3xl  text-center font-bold text-gray-600">Itinéraire {itine.itineraire?.titre}</h1>
        <div className="flex justify-center items-center">
          <img src={logo} className="w-96" alt="Logo" />
        </div>
        <div className="flex justify-between">
          <p className="text-xl font-semibold">Catégorie : <span className="text-red-500">{itine.itineraire?.id_categorie}</span></p>
          <p className="text-xl font-semibold">Durée : <span className="text-red-500">{itine.itineraire?.dure}  jours</span></p>
        </div>
